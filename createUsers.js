...
const subsString = (n, rand) => {
  const arr = Array.from(Array(n), (el) => rand());
  return "[" + arr.join(",") + "]";
};
...
const createUser = () => {
  ...
  return {
    ...
    email,
    subs: subsString(subsNum, rand),
    blocks: subsString(blocksNum, rand),
  };
};

const colSet = new pgp.helpers.ColumnSet(
  ["name", "password", "avatar", "email", "subs", "blocks"],
  {
    table: "users",
  }
);
